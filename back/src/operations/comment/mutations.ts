...
type CommentMutation = WithRequired<MutationResolvers, 'createComment' | 'deleteComment' | 'updateComment'>

export const commentMutations: MutationResolvers = {
    createComment: async (_, {articleId, content}, {user, dataSources: {db}}) => {
        if (!user) {
            return {
                code: 403,
                success: false,
